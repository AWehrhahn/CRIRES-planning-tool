...
rw = 0.9
hist_bins = np.arange(
    int(min(df_plot100["Number of transits"])),
    int(max(df_plot100["Number of transits"])) + binwidth,
    binwidth,
)
ax1.hist(
    [
        df_plot100["Number of transits"].values,
        df_plot150["Number of transits"].values,
        df_plot200["Number of transits"].values,
    ],
    bins=hist_bins,
    rwidth=rw,
    color=[cc[1], cc[7], cc[9]],
    stacked=False,
    label=[
        f"minimum SN = {snr100:.1f}",
        f"minimum SN = {snr150:.1f}",
        f"minimum SN = {snr200:.1f}",
    ],
)
...
hist_bins = np.arange(
    int(min(df_plot100["pl_radE"])),
    int(max(df_plot100["pl_radE"])) + binwidth,
    binwidth,
)
ax2.hist(
    [
        df_plot100["pl_radE"].values,
        df_plot150["pl_radE"].values,
        df_plot200["pl_radE"].values,
    ],
    bins=hist_bins,
    rwidth=rw,
    color=[cc[1], cc[7], cc[9]],
...
rw = 0.9
hist_bins = np.arange(
    int(min(df_plot100["Effective Temperature"])),
    int(max(df_plot100["Effective Temperature"])) + binwidth,
    binwidth,
)
ax4.hist(
    [
        df_plot100["Effective Temperature"].values,
        df_plot150["Effective Temperature"].values,
        df_plot200["Effective Temperature"].values,
    ],
    bins=hist_bins,
    rwidth=rw,
    color=[cc[1], cc[7], cc[9]],
